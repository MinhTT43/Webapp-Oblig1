...
const validerKalender = () => {
   ...
   var dato2 = new Date($('#kalender').val());

   if (Object.prototype.toString.call(dato) === "[object Date]") {
      // it is a date
      if (isNaN(dato.getTime() || isNaN(dato2.getTime))) {  // d.valueOf() could also work
...
const printLines = () => {
   console.log("ERROROEOER")
}

const validerGyldigReise = () => {
   let dato1 = $("#kalender").val();
   let dato2 = $("#kalender2").val();
   console.log(dato1, dato2);

   if (dato1 > dato2) {
      $("#errorDato").html("Sjekk dato");
      console.log(dato1 > dato2);
      return dato1 > dato2;
   } else {
      $("#errorDato").html("");
